...
# 匹配互链块内单个 <a> 标签的正则（含 href 属性）
_RE_CROSSLINK_A = re.compile(
    r'<a\s[^>]*href="([^"]*)"[^>]*>.*?</a>',
    re.IGNORECASE | re.DOTALL,
)

# 匹配互链块整体（含 tag 标记）
_RE_CROSSLINK_BLOCK = re.compile(
    r'<!--\s*======\s*智能互链\s*======\s*-->\s*'
    r'.*?'
    r'<!--\s*======\s*/智能互链\s*======\s*-->',
    re.DOTALL,
...
@require_GET
def api_crosslinks_domain_list(request):
    """
    GET /xiaoying_admin/api/crosslinks/domains/
    扫描所有页面的互链块，返回当前互链中使用的所有域名列表（去重）。
    每个域名包含: url（根域名URL）、display（显示名称）、count（出现次数）
    """
    pages = list(GeneratedPage.objects.all())

    domain_stats = {}  # url -> {display: str, count: int}
    for p in pages:
        html = p.html_content or ''
        block_match = _RE_CROSSLINK_BLOCK.search(html)
        if not block_match:
            continue
        block_html = block_match.group(0)
        for a_match in _RE_CROSSLINK_A.finditer(block_html):
            href = a_match.group(1).strip()
            # 提取 title 属性值作为显示名称
            title_match = re.search(r'title="([^"]*)"', a_match.group(0))
            display = title_match.group(1) if title_match else href
            if href not in domain_stats:
                domain_stats[href] = {'url': href, 'display': display, 'count': 0}
            domain_stats[href]['count'] += 1

    domains = sorted(domain_stats.values(), key=lambda x: -x['count'])
    return JsonResponse({'ok': True, 'domains': domains})


@csrf_exempt
@require_POST
def api_crosslinks_remove_domains(request):
    """
    POST /xiaoying_admin/api/crosslinks/remove-domains/
    从所有页面的互链块中移除指定域名的链接。

    请求: application/json
      {"domains": ["https://example.com/", "https://test.com/"]}
      或 {"remove_all": true}  — 一键清空所有互链

    行为：
      1. 遍历所有页面，解析互链块中的 <a> 标签
      2. 移除 href 匹配指定域名的标签
      3. 如果块内没有剩余链接，删除整个互链块
      4. 返回受影响的页面数 + 移除的链接数
    """
    body, error = parse_json_body(request)
    if error is not None:
    ...
    target_urls = body.get('domains', [])

    if not remove_all and not target_urls:
        return err('请指定要移除的域名，或设置 remove_all=true')

    if remove_all:
        target_urls = None  # 表示全部移除
    else:
        target_urls = set(u.strip() for u in target_urls if u.strip())
        if not target_urls:
            return err('请提供有效的域名列表')
    ...
    for p in pages:
        ...
        # 遍历所有 <a> 标签，移除匹配的
        for a_match in _RE_CROSSLINK_A.finditer(old_block):
            href = a_match.group(1).strip()
            should_remove = remove_all or (href in target_urls)
            if should_remove:
                new_block = new_block.replace(a_match.group(0), '', 1)
                removed_count += 1
        ...
        if not new_block or new_block.strip() in (
            _CROSSLINK_TAG_START.strip(),
            _CROSSLINK_TAG_END.strip(),
            f'{_CROSSLINK_TAG_START}{_CROSSLINK_TAG_END}',
        ):
            # 块已空，删除整个互链块
            new_html = html[:block_match.start()] + html[block_match.end():]
        else:
            # 替换块内容
            new_html = html[:block_match.start()] + new_block + html[block_match.end():]
    ...
    if affected_count == 0:
        msg = '没有找到匹配的互链域名' if not remove_all else '所有页面均无互链，无需清理'
        return JsonResponse({'ok': True, 'message': msg, 'affected_count': 0, 'removed_count': removed_count})

    scope = '全部' if remove_all else f'{len(target_urls)} 个'
    log_operation(request, f'智能互链移除域名（{scope}）: 影响 {affected_count} 个页面，移除 {removed_count} 条链接')

    return JsonResponse({
        'ok': True,
        'message': f'已从 {affected_count} 个页面中移除 {removed_count} 条互链',
